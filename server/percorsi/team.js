...
percorso.get('/', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const teamLeader = await db.prepara("SELECT *, 'leader' as mio_ruolo FROM team WHERE id_utente_leader = ?").tutti(req.utente.id);
    const teamMembro = await db.prepara(`
...
percorso.post('/', autenticaToken, async (req, res) => {
  try {
    ...
    const db = ottieniDb();
    await db.prepara('INSERT INTO team (nome, nome_sponsor, email_sponsor, nome_leader, email_leader, id_utente_leader, stakeholder) VALUES (?, ?, ?, ?, ?, ?, ?)')
    ...
    const tuttiTeam = await db.prepara('SELECT * FROM team WHERE id_utente_leader = ? AND nome = ? ORDER BY id DESC').tutti(req.utente.id, nome);
    ...
    await db.prepara('INSERT INTO membri_team (id_team, email_utente, id_utente, ruolo, iscritto) VALUES (?, ?, ?, ?, 1)')
    ...
    if (email_sponsor && email_sponsor.trim() && email_sponsor.trim() !== req.utente.email) {
      const utenteSponsor = await db.prepara('SELECT id FROM utenti WHERE email = ?').ottieni(email_sponsor.trim());
      await db.prepara('INSERT INTO membri_team (id_team, email_utente, id_utente, ruolo, iscritto) VALUES (?, ?, ?, ?, ?)')
    ...
    if (email_leader && email_leader.trim() && email_leader.trim() !== req.utente.email) {
      const utenteLeader = await db.prepara('SELECT id FROM utenti WHERE email = ?').ottieni(email_leader.trim());
      await db.prepara('INSERT INTO membri_team (id_team, email_utente, id_utente, ruolo, iscritto) VALUES (?, ?, ?, ?, ?)')
    ...
    if (membri && membri.length > 0) {
      for (const email of membri) {
        ...
        const giaAggiunto = await db.prepara('SELECT id FROM membri_team WHERE id_team = ? AND email_utente = ?').ottieni(idTeam, pulita);
        if (giaAggiunto) continue;
        const utenteEsistente = await db.prepara('SELECT id FROM utenti WHERE email = ?').ottieni(pulita);
        await db.prepara('INSERT INTO membri_team (id_team, email_utente, id_utente, ruolo, iscritto) VALUES (?, ?, ?, ?, ?)')
    ...
    if (stakeholder) {
      ...
      for (const email of emailStake) {
        ...
        const giaAggiunto = await db.prepara('SELECT id FROM membri_team WHERE id_team = ? AND email_utente = ?').ottieni(idTeam, email);
        if (giaAggiunto) continue;
        const utenteEsistente = await db.prepara('SELECT id FROM utenti WHERE email = ?').ottieni(email);
        await db.prepara('INSERT INTO membri_team (id_team, email_utente, id_utente, ruolo, iscritto) VALUES (?, ?, ?, ?, ?)')
    ...
    nuovoTeam.membri = await db.prepara('SELECT * FROM membri_team WHERE id_team = ?').tutti(idTeam);
...
// GET /api/team/:id
percorso.get('/:id', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const idTeam = parseInt(req.params.id);
    const teamDati = await db.prepara('SELECT * FROM team WHERE id = ?').ottieni(idTeam);
    if (!teamDati) return res.status(404).json({ errore: 'Team non trovato.' });

    teamDati.membri = await db.prepara(`
      SELECT mt.*, u.nome_utente FROM membri_team mt
      LEFT JOIN utenti u ON mt.id_utente = u.id WHERE mt.id_team = ?
    `).tutti(idTeam);

    const miaIscrizione = await db.prepara('SELECT ruolo FROM membri_team WHERE id_team = ? AND email_utente = ?').ottieni(idTeam, req.utente.email);
    teamDati.mio_ruolo = teamDati.id_utente_leader === req.utente.id ? 'leader' : (miaIscrizione ? miaIscrizione.ruolo : 'membro');

    teamDati.obiettivi = await db.prepara('SELECT * FROM obiettivi WHERE id_team = ? ORDER BY creato_il DESC').tutti(idTeam);
...
percorso.delete('/:id', autenticaToken, async (req, res) => {
  try {
    ...
    const eEditore = teamDati.id_utente_leader === req.utente.id ||
      await db.prepara("SELECT id FROM membri_team WHERE id_team = ? AND email_utente = ? AND ruolo IN ('leader','sponsor')")
    ...
    const obiettivi = await db.prepara('SELECT id FROM obiettivi WHERE id_team = ?').tutti(idTeam);
    for (const ob of obiettivi) {
      const sottoAtt = await db.prepara('SELECT id FROM sotto_attivita WHERE id_obiettivo = ?').tutti(ob.id);
      ...
      await db.prepara('DELETE FROM sotto_attivita WHERE id_obiettivo = ?').esegui(ob.id);
    }
    await db.prepara('DELETE FROM obiettivi WHERE id_team = ?').esegui(idTeam);
    await db.prepara('DELETE FROM membri_team WHERE id_team = ?').esegui(idTeam);
    await db.prepara('DELETE FROM team WHERE id = ?').esegui(idTeam);
...
percorso.get('/:id/calendario', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
...
percorso.post('/:id/membri', autenticaToken, async (req, res) => {
  try {
    ...
    const eEditore = teamDati.id_utente_leader === req.utente.id ||
      await db.prepara("SELECT id FROM membri_team WHERE id_team = ? AND email_utente = ? AND ruolo IN ('leader','sponsor')").ottieni(idTeam, req.utente.email);
    ...
    const giaPresente = await db.prepara('SELECT id FROM membri_team WHERE id_team = ? AND email_utente = ?').ottieni(idTeam, emailPulita);
    if (giaPresente) return res.status(409).json({ errore: 'Membro già presente nel team.' });

    const utenteEsistente = await db.prepara('SELECT id FROM utenti WHERE email = ?').ottieni(emailPulita);
    await db.prepara('INSERT INTO membri_team (id_team, email_utente, id_utente, ruolo, iscritto) VALUES (?, ?, ?, ?, ?)')
      .esegui(idTeam, emailPulita, utenteEsistente ? utenteEsistente.id : null, ruoloPulito, utenteEsistente ? 1 : 0);

    

    const membri = await db.prepara('SELECT mt.*, u.nome_utente FROM membri_team mt LEFT JOIN utenti u ON mt.id_utente = u.id WHERE mt.id_team = ?').tutti(idTeam);
...
percorso.put('/:id/membri/:idMembro', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const idTeam = parseInt(req.params.id);
    const teamDati = await db.prepara('SELECT * FROM team WHERE id = ?').ottieni(idTeam);
    ...
    const ruoloPulito = ['leader', 'sponsor', 'membro', 'stakeholder'].includes(ruolo) ? ruolo : 'membro';
    await db.prepara('UPDATE membri_team SET ruolo = ? WHERE id = ? AND id_team = ?').esegui(ruoloPulito, parseInt(req.params.idMembro), idTeam);
...
percorso.delete('/:id/membri/:idMembro', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const idTeam = parseInt(req.params.id);
    const teamDati = await db.prepara('SELECT * FROM team WHERE id = ?').ottieni(idTeam);
    ...
    const membro = await db.prepara('SELECT * FROM membri_team WHERE id = ? AND id_team = ?').ottieni(parseInt(req.params.idMembro), idTeam);
    if (membro && membro.email_utente === req.utente.email) {
      return res.status(400).json({ errore: 'Non puoi rimuovere te stesso dal team.' });
    }

    await db.prepara('DELETE FROM membri_team WHERE id = ? AND id_team = ?').esegui(parseInt(req.params.idMembro), idTeam);
    const membri = await db.prepara('SELECT mt.*, u.nome_utente FROM membri_team mt LEFT JOIN utenti u ON mt.id_utente = u.id WHERE mt.id_team = ?').tutti(idTeam);
...
percorso.get('/:id/giornaliere/:data', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const micro = await db.prepara(`
...
percorso.patch('/:id/flag-giornaliero/:idMicro', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const { flag } = req.body;
...
percorso.put('/:id/sposta-micro/:idMicro', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
...
percorso.get('/:id/report-giornaliero/:data', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const idTeam = parseInt(req.params.id);
    const membri = await db.prepara('SELECT mt.*, u.nome_utente FROM membri_team mt LEFT JOIN utenti u ON mt.id_utente = u.id WHERE mt.id_team = ?').tutti(idTeam);
