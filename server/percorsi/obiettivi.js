...
percorso.get('/calendario', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const micro = await db.prepara(`
...
percorso.post('/verifica-conflitti', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const oreGiornaliere = await ottieniOreGiornaliereUtente(db, req.utente.id, req.utente.email);
    const conflitti = {};
...
percorso.post('/sistema-conflitti', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const spostamenti = await risolviConflitti(db, req.utente.id, req.utente.email);
...
percorso.put('/sposta-micro/:idMicro', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    ...
    const micro = await db.prepara('SELECT * FROM micro_attivita WHERE id = ?').ottieni(parseInt(req.params.idMicro));
    ...
    const oreDest = await db.prepara(`
    ...
    await db.prepara('UPDATE micro_attivita SET data = ? WHERE id = ?').esegui(nuova_data, micro.id);
...
percorso.get('/giornaliere/:data', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
...
percorso.patch('/flag-giornaliero/:idMicro', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const { flag } = req.body; // 1 = OK, 0 = NOK
    if (flag !== 0 && flag !== 1) return res.status(400).json({ errore: 'Flag deve essere 0 o 1.' });
    await db.prepara('UPDATE micro_attivita SET flag_giornaliero = ? WHERE id = ?').esegui(flag, parseInt(req.params.idMicro));
...
// GET /api/obiettivi
percorso.get('/', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const obiettivi = await db.prepara('SELECT * FROM obiettivi WHERE id_utente = ? AND id_team IS NULL ORDER BY creato_il DESC').tutti(req.utente.id);
    for (const ob of obiettivi) {
      ob.sotto_attivita = await db.prepara('SELECT * FROM sotto_attivita WHERE id_obiettivo = ? ORDER BY id').tutti(ob.id);
      let sommaPct = 0;
...
percorso.post('/', autenticaToken, async (req, res) => {
  try {
    ...
    await db.prepara('INSERT INTO obiettivi (id_utente, id_team, nome, note, data_inizio, ore_totali, data_fine_stimata) VALUES (?, ?, ?, ?, ?, ?, ?)')
      .esegui(req.utente.id, id_team || null, nome, note || '', inizio, ore_totali, fine);
    const ob = await db.prepara('SELECT * FROM obiettivi WHERE id_utente = ? AND nome = ? ORDER BY id DESC').tutti(req.utente.id, nome);
    res.status(201).json({ obiettivo: ob[0] });
  } catch (err) { res.status(500).json({ errore: 'Errore del server.' }); }
});

// PUT /api/obiettivi/:id
percorso.put('/:id', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const { nome, data_inizio, ore_totali, note } = req.body;
    const ob = await db.prepara('SELECT * FROM obiettivi WHERE id = ?').ottieni(parseInt(req.params.id));
    ...
    await db.prepara('UPDATE obiettivi SET nome = ?, note = ?, data_inizio = ?, ore_totali = ?, data_fine_stimata = ? WHERE id = ?')
      .esegui(nome || ob.nome, note !== undefined ? note : (ob.note || ''), nuovoInizio, nuoveOre, calcolaDataFine(nuovoInizio, nuoveOre), ob.id);
    await rigeneraTutte(db, ob.id, nuovoInizio);
    res.json({ obiettivo: await db.prepara('SELECT * FROM obiettivi WHERE id = ?').ottieni(ob.id) });
  } catch (err) { res.status(500).json({ errore: 'Errore del server.' }); }
});

// DELETE /api/obiettivi/:id
percorso.delete('/:id', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const idOb = parseInt(req.params.id);
    const subs = await db.prepara('SELECT id FROM sotto_attivita WHERE id_obiettivo = ?').tutti(idOb);
    for (const sa of subs) {
      await db.prepara('DELETE FROM micro_attivita WHERE id_sotto_attivita = ?').esegui(sa.id);
      await db.prepara('DELETE FROM collaboratori_attivita WHERE id_sotto_attivita = ?').esegui(sa.id);
    }
    await db.prepara('DELETE FROM sotto_attivita WHERE id_obiettivo = ?').esegui(idOb);
    await db.prepara('DELETE FROM obiettivi WHERE id = ? AND id_utente = ?').esegui(idOb, req.utente.id);
    res.json({ messaggio: 'Obiettivo eliminato.' });
  } catch (err) { res.status(500).json({ errore: 'Errore del server.' }); }
});

// POST /api/obiettivi/:id/sotto-attivita
percorso.post('/:id/sotto-attivita', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const idOb = parseInt(req.params.id);
    const obiettivo = await db.prepara('SELECT * FROM obiettivi WHERE id = ?').ottieni(idOb);
    ...
    const rigaSomma = await db.prepara('SELECT COALESCE(SUM(ore_totali), 0) as somma FROM sotto_attivita WHERE id_obiettivo = ?').ottieni(idOb);
    ...
    await rigeneraTutte(db, idOb, obiettivo.data_inizio);
    const saAgg = await db.prepara('SELECT * FROM sotto_attivita WHERE id = ?').ottieni(sa.id);
    if (saAgg) saAgg.micro_attivita = await db.prepara('SELECT * FROM micro_attivita WHERE id_sotto_attivita = ? ORDER BY data').tutti(sa.id);
...
percorso.put('/:idOb/sotto-attivita/:idSa', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const sa = await db.prepara('SELECT * FROM sotto_attivita WHERE id = ? AND id_obiettivo = ?').ottieni(parseInt(req.params.idSa), parseInt(req.params.idOb));
    ...
    await db.prepara('UPDATE sotto_attivita SET nome = ?, ore_totali = ?, ore_per_giorno = ?, giorni_stimati = ?, email_responsabile = ?, percentuale_completamento = ? WHERE id = ?')
    ...
    const obiettivo = await db.prepara('SELECT * FROM obiettivi WHERE id = ?').ottieni(parseInt(req.params.idOb));
    ...
    if ((ore_totali && ore_totali !== sa.ore_totali) || (ore_per_giorno && ore_per_giorno !== sa.ore_per_giorno)) {
      await rigeneraTutte(db, obiettivo.id, obiettivo.data_inizio);
    }

    const agg = await db.prepara('SELECT * FROM sotto_attivita WHERE id = ?').ottieni(sa.id);
    if (agg) agg.micro_attivita = await db.prepara('SELECT * FROM micro_attivita WHERE id_sotto_attivita = ? ORDER BY data').tutti(sa.id);
...
percorso.patch('/sotto-attivita/:idSa/percentuale', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    ...
    await db.prepara('UPDATE sotto_attivita SET percentuale_completamento = ? WHERE id = ?').esegui(percentuale, parseInt(req.params.idSa));
    res.json({ messaggio: 'Percentuale aggiornata.', percentuale });
  } catch (err) { res.status(500).json({ errore: 'Errore del server.' }); }
});

// DELETE /api/obiettivi/:idOb/sotto-attivita/:idSa
percorso.delete('/:idOb/sotto-attivita/:idSa', autenticaToken, async (req, res) => {
  try {
    const db = ottieniDb();
    const idSa = parseInt(req.params.idSa); const idOb = parseInt(req.params.idOb);
    await db.prepara('DELETE FROM micro_attivita WHERE id_sotto_attivita = ?').esegui(idSa);
    await db.prepara('DELETE FROM collaboratori_attivita WHERE id_sotto_attivita = ?').esegui(idSa);
    await db.prepara('DELETE FROM sotto_attivita WHERE id = ? AND id_obiettivo = ?').esegui(idSa, idOb);
    const ob = await db.prepara('SELECT * FROM obiettivi WHERE id = ?').ottieni(idOb);
    if (ob) await rigeneraTutte(db, ob.id, ob.data_inizio);
    res.json({ messaggio: 'Sotto-attività eliminata.' });
  } catch (err) { res.status(500).json({ errore: 'Errore del server.' }); }
});

async function rigeneraTutte(db, idObiettivo, dataInizio) {
  const sottoAtt = await db.prepara('SELECT * FROM sotto_attivita WHERE id_obiettivo = ? ORDER BY id').tutti(idObiettivo);
  for (const sa of sottoAtt) await db.prepara('DELETE FROM micro_attivita WHERE id_sotto_attivita = ?').esegui(sa.id);
  const oreGiorn = {};
  for (const sa of sottoAtt) {
    const micro = generaMicroAttivita(sa, dataInizio, oreGiorn);
    for (const m of micro) await db.prepara('INSERT INTO micro_attivita (id_sotto_attivita, data, ore, etichetta) VALUES (?, ?, ?, ?)').esegui(sa.id, m.data, m.ore, m.etichetta);
